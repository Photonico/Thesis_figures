...
def plot_dielectric_function(suptitle, systems=None, components=None,
    ...
    if isinstance(components, str) or isinstance(components, dict):
        return plot_dielectric_monocomp(
            suptitle, systems, components, layout, expansion_label,
            unit, x_boundary, spectrum_flag, figure_size
        )
    elif isinstance(components, list) and len(components) == 1:
        return plot_dielectric_monocomp(
            suptitle, systems, components, layout, expansion_label,
    ...
    if isinstance(x_boundary, tuple):
        if isinstance(x_boundary[0], tuple) or isinstance(x_boundary[1], tuple):
            return plot_dielectric_function_rescaled(
                suptitle, systems, components, layout, unit,
                x_boundary, spectrum_flag, figure_size
            )
    ...
    if expansion_flag is True:
        for subplot_index in range(2 * len(components)):
            ...
            ax.set_title(
                [f"Real part for {comp_aliases[component_index]}",
                 f"Imaginary part for {comp_aliases[component_index]}"][subplot_index % 2]
            )
            ...
            if subplot_index % 2 == 0:
                for _, data in enumerate(dataset):
                    supercell_thickness, system_thickness = data[6]
                    d_ratio = supercell_thickness / system_thickness
                    energy_real, density_energy_real_source = extract_part(
                        data[1]["density_energy_real"], data[1][data_key], photon_start, photon_end
                    )
                    density_energy_real = density_energy_real_source * d_ratio - d_ratio + 1
                    energy_real, density_energy_real = mask_real(energy_real, density_energy_real, None)
                    ...
                    if var_label == "energy":
                        ax.plot(
                            energy_real, density_energy_real,
                            color=color_sampling(data[2])[1], ls=data[3], lw=data[4], alpha=data[5],
                            label=f"Real part {data[0]}"
                        )
                        energy_starts.append(min(energy_real))
                        energy_ends.append(max(energy_real))
                    else:
                        wavelength_real, density_wl_real = extract_part(
                            energy_to_wavelength(data[1]["density_energy_real"]),
                            data[1][data_key], photon_start, photon_end
                        )
                        ax.plot(
                            wavelength_real, density_wl_real,
                            color=color_sampling(data[2])[1], ls=data[3], lw=data[4], alpha=data[5],
                            label=f"Real part {data[0]}"
                        )
                        wavelength_starts.append(min(wavelength_real))
                ...
                if var_label == "energy":
                    energy_start = min(energy_starts)
                    energy_end = max(energy_ends)
                    ax.plot([energy_start, energy_end], [0, 0],
                            color=color_sampling("grey")[1], linestyle="dashed", alpha=0.8)
                else:
                    wavelength_start = min(wavelength_starts)
                    wavelength_end = max(wavelength_ends)
                    ax.plot([wavelength_start, wavelength_end], [0, 0],
                            color=color_sampling("grey")[1], linestyle="dashed", alpha=0.8)
                ...
                for _, data in enumerate(dataset):
                    supercell_thickness, system_thickness = data[6]
                    d_ratio = supercell_thickness / system_thickness
                    energy_imag, density_energy_imag_source = extract_part(
                        data[1]["density_energy_imag"], data[1][data_key], photon_start, photon_end
                    )
                    density_energy_imag = density_energy_imag_source * d_ratio
                    energy_imag, density_energy_imag = mask_imag(energy_imag, density_energy_imag, None)
                    ...
                    if var_label == "energy":
                        ax.plot(
                            energy_imag, density_energy_imag,
                            color=color_sampling(data[2])[2], ls=data[3], lw=data[4], alpha=data[5],
                            label=f"Imaginary part {data[0]}"
                        )
                    else:
                        wavelength_imag, density_wl_imag = extract_part(
                            energy_to_wavelength(data[1]["density_energy_imag"]),
                            data[1][data_key], photon_start, photon_end
                        )
                        ax.plot(
                            wavelength_imag, density_wl_imag,
                            color=color_sampling(data[2])[2], ls=data[3], lw=data[4], alpha=data[5],
                            label=f"Imaginary part {data[0]}"
                        )
            ...
            if spectrum_flag is True:
                if var_label == "energy":
                    ...
                    ymin, ymax = ax.get_ylim()
                    extent = [ev_min, ev_max, ymin, ymax]
                    ax.imshow(grad, aspect="auto", extent=extent, cmap=energy_cmap,
                              alpha=alpha_grad * 0.3, zorder=-12)
                else:
                    grad = np.linspace(0, 1, 1000).reshape(1, -1)
                    grad = np.vstack([grad] * 10)
                    alpha_vals = np.sin(np.linspace(0, np.pi, 1000)) * 0.4
                    ...
                    ymin, ymax = ax.get_ylim()
                    extent = [380, 750, ymin, ymax]
                    ax.imshow(grad, aspect="auto", extent=extent, cmap=cmap,
                              alpha=alpha_grad * 0.3, zorder=-12)
        ...
        for subplot_index in range(len(components)):
            ...
            for _, data in enumerate(dataset):
                ...
                energy_real, density_energy_real_source = extract_part(
                    data[1]["density_energy_real"], data[1][data_key_real], photon_start, photon_end
                )
                energy_imag, density_energy_imag_source = extract_part(
                    data[1]["density_energy_imag"], data[1][data_key_imag], photon_start, photon_end
                )

                density_energy_real = density_energy_real_source * d_ratio - d_ratio + 1
                density_energy_imag = density_energy_imag_source * d_ratio

                energy_real, density_energy_real, energy_imag, density_energy_imag = mask_real_imag(
                    energy_real, density_energy_real, energy_imag, density_energy_imag, None
                )
                ...
                if var_label == "energy":
                    ...
                    ax.plot(
                        energy_imag, density_energy_imag,
                        color=color_sampling(data[2])[1], ls="dashed", lw=data[4], alpha=data[5],
                        label=f"Imaginary part {data[0]}"
                    )
                    energy_starts.append(min(energy_real))
                    energy_ends.append(max(energy_real))
                    ...
                    wavelength_real, density_wl_real = extract_part(
                        energy_to_wavelength(data[1]["density_energy_real"]),
                        data[1][data_key_real], photon_start, photon_end
                    )
                    wavelength_imag, density_wl_imag = extract_part(
                        energy_to_wavelength(data[1]["density_energy_imag"]),
                        data[1][data_key_imag], photon_start, photon_end
                    )
                    ax.plot(
                        wavelength_real, density_wl_real,
                        color=color_sampling(data[2])[1], ls=data[3], lw=data[4], alpha=data[5],
            ...
            if var_label == "energy":
                plasmon_start = min(energy_starts)
                plasmon_end = max(energy_ends)
                ax.plot([plasmon_start, plasmon_end], [0, 0],
                        color=color_sampling("grey")[1], linestyle="dashed")
            else:
                plasmon_start = min(wavelength_starts)
                plasmon_end = max(wavelength_ends)
                ax.plot([plasmon_start, plasmon_end], [0, 0],
...
def plot_dielectric_function_b(suptitle, systems=None, components=None,
                             layout="horizontal", expansion_label=True,
                             unit=None, x_boundary=(None, None), y_boundary=(None, None),
                             spectrum_flag=None, figure_size = (None,None)):
    ## Help information
